...
{
    public async Task<int> Upsert(BookProgress bookProgress)
    {
        ...
        const string sql = """
                  ...
                  ON CONFLICT (book_id, user_id) DO UPDATE SET
                      current_page_change_at = EXCLUDED.current_page_change_at,
                      current_page = EXCLUDED.current_page
                  WHERE EXCLUDED.current_page_change_at > books_progress.current_page_change_at
                  """;

        return await connection.ExecuteAsync(sql, bookProgress);
    }
}